  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <button onClick={() => filterBy('country', 'México')} className="bg-blue-100 px-4 py-2 rounded border">🇲🇽 México</button>
        <button onClick={() => filterBy('composer', 'Blas Galindo')} className="bg-green-100 px-4 py-2 rounded border">🎼 Blas Galindo</button>
        <button onClick={() => filterBy('composer', 'María Grever')} className="bg-green-100 px-4 py-2 rounded border">🎼 María Grever</button>
        <button onClick={() => filterBy('instruments', 'Voz y piano')} className="bg-yellow-100 px-4 py-2 rounded border">🎤 Voz y piano</button>
      </div>
      {filtered.length > 0 ? (
        <div className="space-y-4">
          {filtered.map(score => (
            <div key={score.title} className="border p-4 rounded bg-white shadow-sm">
              ...
              <a href={score.pdfUrl} download className="text-blue-600 underline mt-2 inline-block">Descargar PDF</a>
            </div>
          ))}
        </div>
      ) : (
